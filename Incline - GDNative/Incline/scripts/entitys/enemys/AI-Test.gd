...
export(float) var distance_threshhold = 3
...
func move_to(target:Vector3):
	path = get_parent().nav.get_simple_path(global_transform.origin, target)
	path_node = 0
...
func _physics_process(delta):
	...
	if state == following && player.global_transform.origin.distance_to(self.global_transform.origin) > distance_threshhold:
		gun.ShootBullet()
